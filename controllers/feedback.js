const { feedbackController } = require("../src/google-client/controllers");
const {
  createImageFromHtml,
  ...
  try {
    const transformedBody = body.map((item) => {
      if (Array.isArray(item.response)) {
        const newResponse = item.response.map((resp, index) => ({
          value: resp,
    ...
    await feedbackController.sendFeedback(transformedBody);
