...
export interface BotMethods {
  ...
  sendPhoto: (chatId: number, fileId: string, options: object, fileOptions?: object) => Promise<any>;
}
...
class LunchHandler {
  ...
  async handleMessage(msg: LunchMessage, botMethods: BotMethods): Promise<boolean> {
    ...
    if (text?.startsWith("Следующая неделя")) {
      await this.handleWeekSelection(chatId, moment().add(1, "week"), botMethods);
      return true;
    }

    if (text === "Ввести дату начала недели") {
      await botMethods.sendMessage(chatId, "Введите дату начала недели (например: 24.02.2026):");
  ...
  private async handleWeekSelection(chatId: number, date: moment.Moment, botMethods: BotMethods) {
  ...
  private async handleDateInput(chatId: number, dateStr: string, botMethods: BotMethods) {
    const parsedDate = moment(dateStr, ["DD.MM.YYYY", "D.M.YYYY", "YYYY-MM-DD"], true);
    if (!parsedDate.isValid()) {
      await botMethods.sendMessage(chatId, "Неверный формат даты. Попробуйте /lunch");
      return;
    }
