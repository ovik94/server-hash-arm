...
const RequestConfigList: Record<string, RequestConfig> = {
  ...
  getOlapCashPayments: {
    method: 'get',
    pathTemplate:
      '/resto/api/v2/reports/olap/byPresetId/5ec59645-62a9-421d-92ae-49c64d72ea57',
  },
};
...
class IikoServerApi {
  ...
  constructor() {
    this.login = config.iiko.serverLogin || '';
    this.pass = config.iiko.serverPassword || '';
    ...
    this.instance.interceptors.response.use(
      ...
      async (err: AxiosError) => {
        ...
        if (
          ...
          const response = await this.createRequest('auth', undefined, {
            login: this.login,
            pass: this.pass,
          });
  ...
  getOlapReport = async (data: Record<string, unknown>) =>
    this.createRequest('getOlapReport', data)
      .then((response) => response?.data)
      .catch((error) => console.log(error));

  getOlapPresets = async () =>
    this.createRequest('getOlapPresets')
      .then((response) => response)
      .catch((error) => console.log(error));

  getOlapCashPayments = async (dateFrom: string) =>
    this.createRequest('getOlapCashPayments', undefined, { dateFrom })
      .then((response) => response?.data)
      .catch((error) => console.log(error));
  ...
  getLunchSales = async (dateFrom: string, dateTo: string) =>
    this.getOlapReport({
      reportType: 'SALES',
      buildSummary: true,
      groupByRowFields: ['Mounth', 'OpenDate.Typed', 'DishCategory'],
      aggregateFields: ['DishDiscountSumInt', 'UniqOrderId'],
      filters: {
        'OpenDate.Typed': {
          filterType: 'DateRange',
